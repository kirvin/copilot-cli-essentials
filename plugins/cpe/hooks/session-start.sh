...
detect_project_tools() {
    ...
    [ -d ".github/workflows" ] && tools="${tools}github-actions "
...
if [ -n "$SKILLS_LIST" ]; then
    ...
    START_MARKER="<!-- CPE_DYNAMIC_SKILLS_START -->"
    END_MARKER="<!-- CPE_DYNAMIC_SKILLS_END -->"

    # The core content payload
    INJECTED_CONTENT="### Available cpe Skills (Auto-Generated)
...
When committing or verifying work, use the ci-cd-pipelines and deployment-strategies skills to guide pipeline changes."; fi)
</INSTRUCTION>"
    ...
    if [ ! -f "$PROJECT_CLAUDE_MD" ]; then
        ...
        if grep -Fq "$START_MARKER" "$PROJECT_CLAUDE_MD" && grep -Fq "$END_MARKER" "$PROJECT_CLAUDE_MD"; then
            EXISTING_CONTENT=$(perl -0777 -ne 'print $1 if /<!-- CPE_DYNAMIC_SKILLS_START -->\n?(.*?)\n?<!-- CPE_DYNAMIC_SKILLS_END -->/s' "$PROJECT_CLAUDE_MD")

            if [[ "$EXISTING_CONTENT" != "$INJECTED_CONTENT" ]]; then
                export CONTENT="$INJECTED_CONTENT"
                perl -i -0777 -pe 's/(<!-- CPE_DYNAMIC_SKILLS_START -->)(.*?)(<!-- CPE_DYNAMIC_SKILLS_END -->)/$1\n$ENV{CONTENT}\n$3/s' "$PROJECT_CLAUDE_MD"
                SHOULD_UPDATE=true
            fi
        else
    ...
    if [ "$SHOULD_UPDATE" = true ]; then
        JSON_TEXT="[SYSTEM UPDATE: cpe Skills list refreshed]
${INJECTED_CONTENT}"
