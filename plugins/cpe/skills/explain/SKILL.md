---
name: explain
description: Use when the user wants a deep explanation of a file, function, or concept — enough to understand it well enough to change it.
---

Explain what the user is asking about — deeply and precisely, for someone who needs to understand it well enough to change it.

## Step 1: Parse the Subject

Determine from the user's message what to explain. It may be:
- A file path (e.g., `src/deploy/rollback.ts`)
- A function or class name (e.g., `handleRollback`)
- A concept or system (e.g., `blue-green deployment`, `CODEOWNERS resolution`)
- A combination (e.g., `the retry logic in src/ci/retry.ts`)
...
```bash
# Find a file
ls src/**/*<name>* 2>/dev/null

# Find a function/class
grep -rn "function <name>\|class <name>\|<name> =" src/ --include="*.ts" --include="*.js" -l
```
