...
4. **Code review (mandatory):** After tests pass and manual verification is done, dispatch the `cpe:code-reviewer` agent via Task tool to review the full diff against the base branch. This step is not optional.
   ...
   - `Skill(cpe:architecting-systems)` - system design, module boundaries
   - `Skill(cpe:managing-databases)` - database work
   - `Skill(cpe:handling-errors)` - error handling
   - `Skill(cpe:writing-tests)` - test quality
   - `Skill(cpe:migrating-code)` - migrations
   - `Skill(cpe:optimizing-performance)` - performance work
   - `Skill(cpe:refactoring-code)` - refactoring
