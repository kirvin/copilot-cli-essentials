...
- **Writing tone and style for config content:** Load `Skill(cpe:writer)` with The Engineer persona for technical docs, or The Educator persona for tutorials and onboarding guides.
- **Documentation:** Load `Skill(cpe:documentation)` when the config involves creating README files, API docs, architecture documentation, or auditing code comments.
