...
Before presenting the plan to the user, dispatch the `cpe:devils-advocate` agent via Task tool to review it:
...
- Load relevant domain skills based on what the plan involves. Evaluate which of these apply and include them in the agent prompt:
  - `Skill(cpe:architecting-systems)` - system design, module boundaries, dependencies
  - `Skill(cpe:managing-databases)` - database schemas, queries, migrations
  - `Skill(cpe:handling-errors)` - error handling patterns
  - `Skill(cpe:writing-tests)` - test strategy and quality
  - `Skill(cpe:migrating-code)` - code migrations, API versioning
  - `Skill(cpe:optimizing-performance)` - performance-sensitive work
  - `Skill(cpe:refactoring-code)` - structural refactoring
