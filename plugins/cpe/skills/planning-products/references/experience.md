...
Define the experience *requirements* before the experience *implementation*. This isn't UI design (that's `Skill(cpe:design)`). This is what the user's journey looks like, what moments matter, and what qualities the feature needs to embody.

The output here feeds into `Skill(cpe:design)` for visual implementation and `Skill(cpe:writing-plans)` for technical implementation.
...
Hand off to `Skill(cpe:design)` for visual implementation. Hand off to `Skill(cpe:writing-plans)` for technical task breakdown.
