...
- **Technical implementation:** `Skill(cpe:writing-plans)` takes the product spec and produces task breakdowns with agent grouping
- **UI/UX craft:** `Skill(cpe:design)` takes the experience requirements and produces the visual implementation
- **Architecture decisions:** `Skill(cpe:architecting-systems)` takes the technical constraints from the spec and produces system design
- **Strategy context:** `Skill(cpe:strategy-writer)` operates upstream, informing the "why this, why now" that feeds into discovery

**Writing tone for specs and product docs:** Use `Skill(cpe:writer)` with The PM persona.
