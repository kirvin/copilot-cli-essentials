---
name: workflow
description: Use when the user wants to debug, fix, or optimize a GitHub Actions workflow.
---
...
- A specific run ID (numeric) — inspect that specific run
- A workflow name — find the latest failing run for that workflow
- No specific target — show all recent failures and ask which to address
...
If the user requested a fix, apply it directly:
