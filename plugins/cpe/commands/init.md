---
description: Initialize or audit GitHub repository best practices for a project
argument-hint: "[--audit] [--fix]"
allowed-tools: Bash, Glob, Grep, Read, Write, Skill, Agent
---
...
If no arguments, default to `--audit` and show what would be set up.
...
CODEOWNERS          ✗ Missing — run /cpe:init --fix to create
PR Template         ✓ Present
Dependabot          ⚠ Missing npm ecosystem
CI/CD Pipeline      ✓ GitHub Actions detected
Issue Templates     ✗ Missing

Run /cpe:init --fix to create missing configurations.
```
