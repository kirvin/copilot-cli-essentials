---
description: Security and dependency audit — vulnerabilities, secrets, and outdated packages
argument-hint: "[--deps|--secrets|--all]"
...
```bash
# Check for common secret patterns (non-destructive grep)
# API keys, tokens, passwords in tracked files
git log --all --oneline | head -5  # context only
...
Include remediation commands for each finding. If nothing found, confirm the project is clean.
