...
tools: ["bash", "glob", "rg", "view", "incident-response", "deployment-strategies"]
---
...
Apply the `incident-response` skill. **Hypothesis-driven.** Form 1–3 hypotheses based on recent changes. Test each:
...
Apply the `deployment-strategies` skill. **Bias for rollback.** If the incident correlates with a recent deploy, roll back first and investigate after.
