...
tools: ["bash", "glob", "rg", "view", "reading-logs", "incident-response"]
---
...
- Any self-hosted URL → Jenkins

## Step 2: Load CI-Specific Reference

Use the `reading-logs` skill which routes to fetch commands, error patterns, and filtering strategies specific to the detected CI system.
...
If the failure is environmental (flaky network, disk full, OOM) vs. code-caused, say so explicitly.

## Efficiency Rules

- Load partial logs, not entire files
- Use system-specific "failed only" fetch before full log
- Stop investigating once root cause is confirmed
- If the same error repeats N times, report it once with count
- If logs aren't accessible, ask the user to paste the relevant section
