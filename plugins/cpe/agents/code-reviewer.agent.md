...
tools: ["bash", "glob", "rg", "view", "ide-get_diagnostics", "security-scanning", "code-ownership", "ci-cd-pipelines"]
---
...
- **Security**: Apply `security-scanning` skill — injection, auth, secrets, CVEs in new deps
...
Apply the `security-scanning` skill to the diff:
...
For any changes to `.github/workflows/`, apply the `ci-cd-pipelines` skill and check:
...
- **CODEOWNERS**: New modules or files without an owner? Apply `code-ownership` skill.
...
For IDE diagnostics: call `ide-get_diagnostics` with specific file URIs for each changed file (e.g., `file:///path/to/file.ts`). Never call without a URI.
